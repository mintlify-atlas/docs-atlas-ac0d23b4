---
title: "Element System: IDs, Types, and Lifecycle"
...
// Anchor elements at offset 10 from the current scroll position
ElemAnchor(10, hud_ids, 2);
```
...
To un-anchor elements, call `ElemAnchor(0, ids, count)` — this resets their anchor and restores their `y` position to the anchor value that was stored.
...
By default, elements are drawn to the back-buffer **before** the `on_every` callback runs (`ORDER_ELEM_FIRST`). This means anything you draw inside `on_every` using immediate-mode functions appears on top of retained elements.

Call `SetRenderingOrder` to swap the order:

```c
// Elements render first, then on_every draws on top (default)
SetRenderingOrder(ORDER_ELEM_FIRST);   // ORDER_ELEM_FIRST = 1

// on_every callback runs first, then elements render on top
