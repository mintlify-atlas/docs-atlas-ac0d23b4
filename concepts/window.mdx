---
title: "Window Lifecycle and the Render Loop"
sidebarTitle: "Window"
...
2. **Call `on_every(tick, delta_time)`** — your per-frame callback, where `tick` is an incrementing `unsigned long long` frame counter (starting at 0) and `delta_time` is a `float` representing elapsed seconds since the previous frame.
...
<Note>
The `on_every` callback signature uses `int tick` in the public C header, but internally the tick counter is `unsigned long long`. For practical purposes the `int` type is sufficient unless you need extremely long-running sessions.
</Note>

The rendering order of elements relative to the `on_every` callback can be changed with `SetRenderingOrder`. By default, elements are rendered first (`ORDER_ELEM_FIRST`).
