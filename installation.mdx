...
<Steps>
  ...
  <Step title="Build the shared library">
    ...
    The Makefile compiles all sources with `g++ -std=gnu++17 -fPIC -shared` and links against `-lX11 -lGL`. The default optimization level is `-O2`; you can override it with the `O=` variable:
...
<Note>
`CreateWindow` returns `0` on success. It returns `1` if `XOpenDisplay` fails (no display available), `2` if the default X11 bitmap font cannot be loaded, and `3` if a window is already open. Values `128`–`143` indicate that Xlib was unable to allocate one of the 16 palette colors.
</Note>
