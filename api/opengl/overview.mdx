---
title: "OpenGL Element API: GLX Surface Integration"
sidebarTitle: "OpenGL"
description: "Create a GLX-backed OpenGL surface element, manage contexts, synchronize rendering, and convert TGA images to OpenGL textures."
---
...
When `border` is a valid palette index (≥ 0), the element's stored dimensions are expanded by 1 pixel on all sides (`w + 2`, `h + 2`) and LWXGL draws a 1-pixel border in that color when compositing the element. Pass `CLR_NONE` (`-1`) for no border.
