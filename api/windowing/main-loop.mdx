...
2. **Render frame** — the back-buffer is cleared, then `on_every` and element rendering are interleaved according to the rendering order set by `SetRenderingOrder`:
   ...
   - `ORDER_ELEM_FIRST` (`1`): elements are rendered to the back-buffer **before** `on_every` is called.
   The completed back-buffer is then copied to the window with `XCopyArea`.
